...
var audio = {
    ...
    init: function(){
        if(game.audio){
            this.pool['music1'] = {
                audio: this.createChannel('media/audio/loop.ogg'),            
                volume: 0.7,
                loop: true,
            };
            
            this.pool['button'] = {
                audio: this.createChannel('media/audio/click-ack-hi.ogg'),
                volume: 0.9,
            };

            this.pool['select_unit'] = {
                audio: this.createChannel('media/audio/click-ack-low.ogg'),
                volume: 0.9,
            };
            
            this.pool['click'] = {
                audio: this.createChannel('media/audio/general-click.ogg'),
                volume: 0.9,
            };

            this.pool['gold'] = {
                audio: this.createChannel('media/audio/jrpg-style-ack.ogg'),
                volume: 0.9,
            };      
        }
    },
            ...
            if(this.pool[args.sound]){
                ...
                if(create_channel){
                    if(this.channels.length < this.max_channels){
                        ...
                        temp_channel.setAttribute('type','audio/ogg');
